<div class="space-y-12">
    ...
    <section class="rounded-4xl bg-linear-to-br from-white/90 via-white/70 to-white/40 px-6 py-10 shadow-[0_20px_45px_rgba(0,0,0,0.1)] backdrop-blur-sm sm:px-8">
        <div class="mb-6 flex items-center justify-between">
            <h2 class="text-2xl font-bold text-slate-900">Daftar Mahasiswa Wisuda</h2>
            <span class="rounded-full border border-slate-200 bg-white px-3 py-1 text-xs font-semibold text-slate-600">{{ count($graduates) }} Mahasiswa</span>
        </div>

        <div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
            @foreach ($graduates as $graduate)
                <article class="group flex flex-col items-center rounded-3xl border border-slate-200/70 bg-white/90 px-6 py-7 text-center shadow-[0_16px_30px_rgba(15,23,42,0.08)] transition duration-300 hover:-translate-y-1 hover:shadow-[0_22px_40px_rgba(15,23,42,0.15)]">
                    <div class="relative">
                        <img
                            src="{{ $graduate['photo'] }}"
                            alt="{{ $graduate['name'] }}"
